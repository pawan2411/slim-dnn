...
class SeluLayer(Layer):
    """
        Layer for scaled exponential linear unit transformation. This class extends base Layer class.
        
        init params:
          alpha: Alpha dropout restores the original values of mean and variance thereby preserving the self-normalizing
            property
          scale:>1 to ensure a slope larger than one for positive net inputs. default value taken from tf   
                library(https://www.tensorflow.org/api_docs/python/tf/keras/activations/selu)
    ...
    def fprop(self, inputs):
        '''
            If x(input) is greater than zero, then the output value will be 'y = scale*x'. But if 
             input is less than zero, then 'y = scale*alpha*(e^x-1)' (reference: selu paper https://arxiv.org/abs/1706.02515)
        Args:
            inputs: Array of layer inputs of shape (batch_size, input_dim).
        ...
        if (inputs.all() > 0):
            ...
            return self.scale * (self.alpha * np.exp(inputs) - self.alpha)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        Differentiation of above (input)
        ...
        if (inputs.all() > 0):
            scale_mat = np.ones(outputs.shape[0], dtype = int) 
            return self.scale * scale_mat
        else:
            return np.multiply(self.scale * self.alpha,np.exp(grads_wrt_outputs))
...
class GeluLayer(Layer):
    """Layer for gaussian error linear unit transformation."""
    def fprop(self, inputs):
        """
        For 'x'(inputs)
        'y = 0.5*x*(1+tanh(sqrt(2/pi)*(x+0.044715*x^3)'   
        reference: https://www.tensorflow.org/api_docs/python/tf/keras/activations/gelu
        Args:
            inputs: Array of layer inputs of shape (batch_size, input_dim).
...
class IsrlLayer(Layer):
    """Layer for inverse square root linear unit transformation.""" 
    ...
    def fprop(self, inputs):
        ...
        x >= 0 and y = x*(1/sqrt(1+alpha*x^2)) if x > 0
        reference: https://arxiv.org/abs/1710.09967, https://openreview.net/pdf?id=HkMCybx0-
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        """
        return grads_wrt_outputs if inputs.all() >= 0 else grads_wrt_outputs*(1/np.sqrt(1+self.alpha*inputs**2))**3
